...
__global__ void scaleKernel(float *d_inp, int len, float *d_out, float scale)
{
...
__global__ void vecAddKernel(float *d_inp1, int len1, float *d_inp2, int len2, float *d_out)
{
	const int i = blockIdx.x*blockDim.x + threadIdx.x;
	if (i<len1)
...
__global__ void compProdKernel(float *d_inp1, int len1, float *d_inp2, int len2, float *d_out)
{
	const int i = blockIdx.x*blockDim.x + threadIdx.x;
	if (i<len1)
		d_out[i] = d_inp1[i] * d_inp2[i];
...
void vecAdd(float *inp1, int len1, float *inp2, int len2, float *out)
{
	...
	else
	{
		...
		vecAddKernel << <(len1 / TPB), TPB >> >(d_inp1, len1, d_inp2, len2, d_out);
...
void compProd(float *inp1, int len1, float *inp2, int len2, float *out)
{
	...
	else
	{
		...
		compProdKernel << <(len1 / TPB), TPB >> >(d_inp1, len1, d_inp2, len2, d_out);
